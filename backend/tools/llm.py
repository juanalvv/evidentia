    ...
    async def complete(self, prompt: str, model: str, max_tokens: int = 1024) -> str:
        ...
        try:
            response = await self.client.chat.completions.create(
                model=model_id,
                messages=[
                    {"role": "system", "content": "You are a precise academic research assistant. Follow the user's instructions exactly. DO NOT include any conversational filler, preambles, introductory remarks, or explanations. If the user asks for JSON, return ONLY the raw JSON object. If the user asks for a query, return ONLY the query text."},
